"""Process the v2 chirp recording (Ftest2.csv) into Bode magnitude plots.
...
Reads the .npy cache written by cache_dxd_v3.py, not the 744 MB CSV directly.
"""
...
CACHE_DIR = V3_DIR / "analysis" / "cache"
PLOT_DIR = V3_DIR / "analysis" / "plots"
...
# v3 names its accelerometer axes X/Y/Z where v2 used AI 1/2/3;
# column order is unchanged (X->0, Y->1, Z->2).
...
def make_general_visualization(accel, t0, clock, out_path):
    ...
    ruler.set_title("v2 chirp recording (Ftest2.csv) -- full record, all axes, "

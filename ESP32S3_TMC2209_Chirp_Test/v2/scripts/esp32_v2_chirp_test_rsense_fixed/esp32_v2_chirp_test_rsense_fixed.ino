 ...
 * -- the sequence previously validated on real hardware (full RUN completed
 * segment-by-segment on schedule, driver connected/configured correctly) --
 * with two changes, both about getting back to that known-good baseline
 * rather than anything explored since:
 *
 *   1. R_SENSE_OHM corrected to 0.11, the resistor actually fitted on this
 *      board, not v2's 0.03 (a different board's value; see the fix applied
 *      to Microstepping Test Data/v5 and v6 first). At this sketch's
 *      CURRENT_RMS_MA=400, 0.03 only delivers ~153 mA (38%); 0.11 delivers
 *      ~398 mA, essentially exactly the target. Nothing else about the
 *      sequence -- sweep range, notch, amplitude -- is touched: this is
 *      deliberately the same run that worked before, not the widened-notch
 *      or restructured-sweep variants tried afterward (not kept; see
 *      ESP32S3_TMC2209_Chirp_Test/v3 in git history for that investigation
 *      and the two hardware incidents that ended it).
 *   2. Waits indefinitely for the TMC2209 to be connected, instead of
 *      trying once at boot and giving up. configureDriver() itself is
 *      unchanged (still its own 10-attempt/200ms burst per call); setup()
 *      now wraps that in an outer loop that keeps retrying once a second
 *      until it succeeds, since this sketch is meant to be flashed before
 *      the driver is powered and connected. Once detected, it waits 7 s
 *      (audible/visible margin to notice it before motion starts) and then
 *      runs automatically -- CHECK/RUN (repeat)/ABORT remain available
 *      afterward.
 *
 * An onboard RGB LED phase indicator is also added (v2 had none): yellow
 * while waiting for/configuring the driver, blue for the 7 s countdown
 * once detected, green while actively running, cyan on completion, red on
 * failure/abort. Serial has proven unreliable to watch live on this rig, so
 * the LED is the primary way to tell what phase this is in from across the
 * room.
 *
 * Everything below this point is v2's own header, unchanged:
 *
 * ---
 *
...
namespace chirp_v2_rsense_fixed {
...
void printPlan() {
  Serial.println("# v2 chirp plan (R_SENSE fixed): cruise amplitude + smooth "
                  "notch (no e_max)");
...
using namespace chirp_v2_rsense_fixed;
...
void setup() {
  ...
  Serial.println("# ESP32-S3/TMC2209 v2 chirp (R_SENSE fixed to 0.11), "

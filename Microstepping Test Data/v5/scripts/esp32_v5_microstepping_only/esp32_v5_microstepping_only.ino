/*
 * ESP32-S3/TMC2209 v5 microstepping-only sequence.
 *
 ...
 *
 * Just the microstep positional-fidelity measurement, with none of the
 * range/centering/trajectory machinery from
 * esp32_v5_centered_mres_campaign: for each MRES in 1, 4, 16, 32, a small
 * negative-then-return marker, then the unchanged 15-cycle one-microstep
 * forward/return oscillation (1 s dwell each side) that the rest of this
 * project has used throughout. No 25 mm/12 mm trajectory legs, no DIRECT vs
 * INDIVIDUAL command-mode comparison, no throughput preflight (that only
 * mattered for the INDIVIDUAL trajectory mode, which does not exist here).
 *
 ...
 * full steps (well under 1 mm); the oscillation itself is +/-1 microstep.
 * That is safe to run from wherever the stage happens to be sitting when
 * this boots, which is why it starts immediately rather than waiting for a
 * range to be established the way esp32_v5_centered_mres_campaign does.
 * Position 0 is simply wherever the stage was at boot.
 *
 * R_SENSE_OHM is 0.11, the resistor actually fitted on this board (not the
 * 0.03 used by every pre-v5 sketch, which under-delivered current by
 * roughly 3x). Chopper mode is forced to SpreadCycle with interpolation off
 * and verified by readback, for the same reason as v4/v5: the TMC2209's
 * power-on defaults corrupt commanded microsteps badly enough to invalidate
 * this measurement.
 *
...
bool runCampaign() {
  ...
  logEvent("CAMPAIGN_START", "BASELINE", 0, 1, 0, "microstepping_only;no_trajectories");
...
void setup() {
  ...
  Serial.println("# ESP32_V5_MICROSTEPPING_ONLY (no trajectories; auto-runs at boot)");

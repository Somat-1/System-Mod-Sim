"""Shared loading and styling for the v5 EL5101 IDS captures.

Same TwinCAT "YT Scope Project" export format used throughout this project
(see v4/analysis/scripts/ids_common.py): a tab-separated header block, then
`index<TAB>counter` rows at the rate given by `SampleTime[ms]`. The counter is
a raw UINT32 count; the repository's established scale is 1 nm per count, so a
count is a nanometre and 1 full step of the stage is 10 um (2 mm lead / 200
full steps per revolution -- matches v5's own FULL_STEPS_PER_MM=100).
"""
...
V5_DIR = Path(__file__).resolve().parents[2]
ANALYSIS_DIR = V5_DIR / "analysis"
...
def find_marker_onsets(t, pos_um, threshold_um=50.0, min_gap_s=1.5):
    """Marker moves are negative-then-return dips far larger than any
    microstep, so a plain threshold finds them."""

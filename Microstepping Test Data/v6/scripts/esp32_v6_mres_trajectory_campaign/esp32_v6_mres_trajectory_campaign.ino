/*
 * ESP32-S3/TMC2209 v6 MRES and 10 mm trajectory campaign.
 *
 * Same structure as v4's esp32_v4_mres_trajectory_campaign (this file was
 * forked from it directly), with two corrections established since:
 *
 *   - R_SENSE_OHM is 0.11, the resistor actually fitted on this board, not
 *     the 0.03 v4 used (a different board's value). 0.03 under-delivered
 *     current by roughly 3x (~122 mA actual against the 360 mA requested
 *     here); 0.11 actually delivers close to 360 mA.
 *   - TRAJECTORY_FULL_STEPS is 10 mm each way, not v4's 25 mm. The stage's
 *     actual travel was since measured (tmp_v4_manual_range_sweep, in v4)
 *     at 44 mm total -- a 25 mm one-way leg does not fit at all.
 *
 * Like v4, this sketch still has no range-finding or centering step and
 * starts moving automatically at boot: 10 mm each way only fits safely if
 * the stage is already sitting with at least 10 mm of clearance on both
 * sides before it is flashed. Prefer v5 (esp32_v5_centered_mres_campaign) if
 * that clearance isn't already known -- it establishes and centers on the
 * range interactively before anything moves, though at a shorter 12 mm leg
 * and split into both a positive and a negative excursion per speed/mode
...
constexpr uint32_t TRAJECTORY_FULL_STEPS = 10UL * FULL_STEPS_PER_MM;
...
bool runDirectTrajectory(const char *rateName, uint32_t rateMilliHz) {
  ...
  logEvent("BLOCK_START", "DIRECT", rateMilliHz, 1, 0,
           "10mm;one_command_per_leg");
  if (!moveFullStepsDirect(TRAJECTORY_FULL_STEPS, rateMilliHz)) return false;
  logEvent("ENDPOINT", "DIRECT", rateMilliHz, 1, 0, "plus_10mm");
...
bool runIndividualTrajectory(const char *rateName, uint32_t rateMilliHz) {
  ...
  logEvent("BLOCK_START", "INDIVIDUAL", rateMilliHz, 1, 0,
           "10mm;1000_one_full_step_commands_per_leg");
  if (!runIndividualLeg(1, rateMilliHz, "outbound")) return false;
  logEvent("ENDPOINT", "INDIVIDUAL", rateMilliHz, 1, 0, "plus_10mm");
  if (!cancellableDwell(ENDPOINT_DWELL_MS)) return false;

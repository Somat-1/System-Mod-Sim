 ...
 * retrofitted into the baseline sketch). Every MRES value, oscillation,
 * marker, trajectory rate, dwell, and separation is identical to the
 * baseline so the two datasets stay comparable. The only behavioural
 * difference is the driver configuration: this sketch explicitly enables
 * SpreadCycle (driver.en_spreadCycle(true)) and disables MicroPlyer
 * step interpolation (driver.intpol(false)) for the whole sequence, instead
 * of leaving the TMC2209 at its power-on default (StealthChop, interpolation
 * on). No MicroPlyer-enabled runs are included here; those are being run as
 * a separate additional sequence.
 *
...
constexpr uint32_t ENDPOINT_DWELL_MS = 2000;
constexpr uint32_t EXPERIMENT_SEPARATION_MS = 10000;
...
bool configureDriver() {
  ...
  driver.iholddelay(0);
  driver.TPOWERDOWN(0);
  // SpreadCycle for the whole sequence, MicroPlyer interpolation off --
  ...
  for (uint8_t attempt = 0; attempt < 10; ++attempt) {
    if (driver.test_connection() == 0 && driver.version() == 0x21) {
      ...
      Serial.println("# DRIVER_MODE,spreadcycle=1,intpol=0");
      return true;
    }
    delay(150);
  }
...
bool runOscillation() {
  const int64_t startPositionU32 = positionU32;
  ...
  if (positionU32 != startPositionU32) return false;
...
bool runCampaign() {
  ...
  logEvent("CAMPAIGN_START", "SPREADCYCLE", 0, 1, 0,
           "spreadcycle_enabled;interpolation_disabled");
  ...
  for (size_t mresIndex = 0; mresIndex < MRES_COUNT; ++mresIndex) {
    ...
    logEvent("RUN_CONFIG", "SPREADCYCLE", 0, 1, 0, "mres_configured");
    ...
    for (size_t mode = 0; mode < 2; ++mode) {
      for (size_t rateIndex = 0; rateIndex < RATE_COUNT; ++rateIndex) {
        if (!cancellableDwell(EXPERIMENT_SEPARATION_MS)) return false;
    ...
    logEvent("RUN_COMPLETE", "SPREADCYCLE", 0, 1, 0, "origin");
    if (!checkOrigin("run_complete")) return false;
  }

  currentBlock = "CAMPAIGN";
  if (!cancellableDwell(CAMPAIGN_TAIL_MS)) return false;
  logEvent("CAMPAIGN_COMPLETE", "SPREADCYCLE", 0, 1, 0, "origin");
  running = false;
  colour(0, 22, 22);
  return true;
}
...
void setup() {
  ...
  Serial.println("# ESP32_V4_MRES_TRAJECTORY_CAMPAIGN_SPREADCYCLE");

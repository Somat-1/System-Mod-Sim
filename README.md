...
8. **Use the installed geometry.** The screw is 192 mm long, approximately 170 mm is usable, the stage travel is 150 mm, and the installed lead-accuracy class is IT3. Both screw segment stiffness pairs and the polar inertia are derived from one section and one declared nut datum, so the torsional and axial pairs cannot describe different screws; mass uses the nominal diameter and every stiffness uses the root diameter.
9. **Keep the command-grid question open.** The 1/16 divisor is adequate for quantization noise and irrelevant to microstep nonlinearity, but one 312.5 nm microstep is larger than the entire detent equilibrium error, so positional pre-distortion against a position-periodic term is not expressible on this grid. The builder reports the required divisor on every run.
...
- Every main and memory plateau uses a damping-derived dwell; metrics use the settled final 20 ms rather than the ringing transient. The dwell is the maximum over a 100 ms floor, the detent-softened drive estimate, the executed and interface-propagated axial estimates, and the measured relative-mode estimate. The measured branch currently governs, which makes the plateaus several times longer than the floor; resolving the damping question in E.7 is what would shorten them.
- Each nonlinear case is also run with the detent torque removed. The paired ablation separates the settled window into a detent term and a friction term, because case 0 is frictionless but not force-free and its settled deviation is a pure detent result.

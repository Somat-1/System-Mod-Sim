...
def main() -> None:
    # ---- Same two eigensolves as the Split Method (Stage 1) ----
    ...
    md.append(
        ...
        "displacement using the lead-screw's own kinematic relationship, then **all 6 DOFs are "
    ...
    # ---- Step 1: kinematic scaling ----
    md.append("\n## Step 1 -- Kinematic Scaling of the Rotational Rows\n")
    md.append(
        "The lead screw's ideal kinematic relationship (ball-screw/nut, no slip) ties axial "
    ...
    md.append("\n### Scaled eigenvector matrix $\\phi_{0,\\text{scaled}}$ -- Set A\n")
    md.append(md_matrix(phi0_s, STATE_LABELS, mode_cols))
    md.append("\n### Scaled eigenvector matrix $\\phi_{1,\\text{scaled}}$ -- Set B\n")
    md.append(md_matrix(phi1_s, STATE_LABELS, mode_cols))

    # ---- Step 2 (user's Step 3): single unaliased 6-DOF Global MAC ----
    ...
    md.append("\n### MAC$_{\\text{scaled}}$ (rows: Set A, cols: Set B)\n")
    md.append(md_matrix(mac_scaled, a_mode_labels, b_mode_labels, fmt="{:.4f}"))

    # ---- Step 3 (user's Step 4): master pairing list ----
    master_pairs = []
    for i in range(N):
    ...
    duplicated_b = sorted({b for b in b_used if b_used.count(b) > 1})

    md.append("\n## Step 3 -- Master Pairing List\n")
    md.append(
        "Row-wise argmax of $\\mathrm{MAC}_{\\text{scaled}}$ -- one Set-B mode picked per "
        "Set-A mode:\n"
    )
    ...
    if duplicated_b:
        ...
        md.append(
            ...
            "one Set-A mode above (A-mode5 and A-mode6 both peak at B-mode4 here -- 0.9796 and "
            "0.8463 respectively). Preserving all 6 DOFs together removes the *severe* aliasing "
            "seen in the Split Method's translational block (that whole block was $\\geq 0.93$ "
            "with no separation at all), but it does not by itself guarantee a strict one-to-one "
            "assignment -- resolving that fully would need an assignment algorithm (e.g. "
            "Hungarian/linear-sum-assignment) on top of the row-wise argmax, which is not run "
            "here. Both rows are still carried through Steps 4-6 below as given, matching how "
            "the Split Method handled its own duplicate (B-mode6 claimed by two rows in the "
            "rotational sub-MAC).\n"
        )

    # ---- Step 4 (user's Step 5): sign alignment ----
    signs = []
    for i, j in master_pairs:
    ...
    md.append(
        "For each pair $(l, k)$ above, compute $S = \\vec{\\phi}_{A,\\text{scaled},l}^{\\mathsf{T}}"
        "\\vec{\\phi}_{B,\\text{scaled},k}$ over the full 6-element scaled vectors. If $S < 0$, "
        "`eigh` returned mode $k$ of Set B with the opposite overall sign from mode $l$ of Set "
        "A; multiply column $k$ of $\\phi_{1,\\text{scaled}}$ by $-1$ before using it in Step 6 "
        "(applied per pair, not per column, since B-mode4 above is reused by two different "
        "pairs and is not guaranteed to need the same sign both times):\n"
    )
    ...
    # ---- Step 5 (user's Step 6): unified single COMAC loop over all 6 DOFs ----
    n_dof = N
    ...
    md.append(
        "\nReading these against the Split Method's split-vector result: $\\theta_{sb}$ (support "
        f"bearing ring) is now clearly the lowest of all 6 DOFs at {comac[3, 3]:.3f} -- and it is "
        "the one DOF the LuGre bearing friction torque acts on directly, so this is the cleanest "
        "physical signal either method has produced. $\\theta_c$ is next-lowest "
        f"({comac[1, 3]:.3f}). Both translational DOFs recover from the Split Method's near-zero "
        f"reading to $x_s = {comac[4, 3]:.3f}$ and $x_n = {comac[5, 3]:.3f}$ -- consistent with "
        "that earlier near-zero result being an artifact of leaving the eigenvector sign "
        "unresolved before summing (Step 4 above is exactly the fix), not a genuine finding "
        "about $x_s$/$x_n$. $\\theta_m$ and $\\theta_s$ score highest "
        f"({comac[0, 3]:.3f}, {comac[2, 3]:.3f}), i.e. least reshaped by the added friction.\n"
    )
    ...
    md.append(
        ...
        "- This MAC$_{\\text{scaled}}$ is **not** comparable cell-by-cell with the Split "
        "Method's MAC$_{\\text{rot}}$/MAC$_{\\text{trans}}$: those were computed on sub-vectors "
        "with $\\theta$ still in raw radians (mass-normalized units, not physically scaled), "
        "so the resulting best-match pairs differ (e.g. A-mode2 pairs to B-mode1 in the Split "
        "Method's rotational sub-MAC but to B-mode1 here too by coincidence of row 2, while "
        "A-mode1 moves from B-mode1 to B-mode2) -- each method's pairing is only self-consistent "
        "within itself.\n"
        "- As in the Split Method, sign is otherwise arbitrary per raw mode "
        "(`eigh` guarantees $\\phi^{\\mathsf T} M \\phi = I$, not a consistent sign); Step 4 "
        "above resolves it only for the 6 pairs actually used in Step 5, not for the full "
        f"raw $\\phi_0$/$\\phi_1$ matrices.\n"
    )
